...
//add phase_ri
int16_t phase_ri[14] = {16383, -28378, 25101, -21063, 30791, -11208, 32767, 0, 30791, 11207, 25101, 21062, 16383, 28377};
...
int rx_sss(PHY_VARS_UE *ue,int32_t *tot_metric,uint8_t *flip_max,uint8_t *phase_max)
{
  ...
  uint8_t i;
  int32_t pss_ext[4][72];
  int32_t sss0_ext[4][72],sss5_ext[4][72],sss05_ext[4][144]; //add sss05
  uint8_t Nid2 = ue->common_vars.eNb_id;
  uint8_t flip,phase;
  uint16_t Nid1;
  int16_t *sss0,*sss5,*sss05; //add sss05
  LTE_DL_FRAME_PARMS *frame_parms=&ue->frame_parms;
  int32_t metric;
  int16_t *d0,*d5,*d05;
	FILE* fp,fp2;
  ...
  *tot_metric = -99999999;
  ...
  sss05 = (int16_t*)&sss05_ext[0][5]; //add sss05
...
time_diff = rdtsc_oai();
	...
	if(flag_sswrite == 0)
	{
		...
		for (flip=0;flip<3;flip++)
		{
			...
			{
				d0 = &d0_sss[62*(flip + (Nid1*3))];
          		d5 = &d5_sss[62*(flip + (Nid1*3))];
		  
				for (i=0; i<62; i++)
				{
					fprintf(fp, "%d, %d,", d0[i], -d0[i]);
				}
				fprintf(fp,"\n");
				for (i=0; i<62; i++)
				{
					fprintf(fp, "%d, %d,", d5[i], -d5[i]);
				}
				fprintf(fp,"\n");
      ...
      for (Nid1 = 0 ; Nid1 <= 167; Nid1++) {  // 168 possible Nid1 values
        ...
        for (i=0; i<62; i++) {
          metric += (int16_t)(((d0[i]*((((phase_re[phase]*(int32_t)sss0[i<<1])>>19)-((phase_im[phase]*(int32_t)sss0[1+(i<<1)])>>19)))) +
                               (d5[i]*((((phaseuiidsi_re[phase]*(int32_t)sss5[i<<1])>>19)-((phase_im[phase]*(int32_t)sss5[1+(i<<1)])>>19))))));
        }
...
//SSS mod
  //stop_meas(&sss_cal);
	time_diff = rdtsc_oai() - time_diff;
//  	printf("[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[time = %f \n Value = (%d,%d,%d)\n", time_diff/(cpuf*1000.0),tot_metric,phase_max,flip_max);
		fp = fopen("data1origin.txt","w")
		fprintf(fp, "%f \n Value = (%d,%d,%d)\n", time_diff/(cpuf*1000.0),tot_metric,phase_max,flip_max);
		fclose(fp);
...
/*	
  ...
  //start_meas(&sss_cal);
  time_diff = rdtsc_oai();
  //d05 = (int*) malloc(sizeof(int)*124*504);
  
	for (flip=0; flip<2; flip++) {		//	d0/d5 flip in RX frame
	  for (phase=0; phase<14; phase++) { // phase offset between PSS and SSS
		for (Nid1 = 0 ; Nid1 <= 167; Nid1++) {	// 168 possible Nid1 values
		  metric = 0;
		  ...
		  // This is the inner product using one particular value of each unknown parameter
		  for (i=0; i<124; i++) {
			metric += (int16_t)(((d05[i]*((phase_ri[phase]*(int32_t)sss05[i<<1])>>19))+	 (d05[i]*((phase_ri[phase]*(int32_t)sss05[62+i<<1])>>19))));
		  }
  ...
  //SSS mod
	//stop_meas(&sss_cal);
	time_diff = rdtsc_oai() - time_diff;
	printf("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<time2 = %f \n Value = %d\n", time_diff/(cpuf*1000.0),tot_metric);
*/ 

//simd
	int32_t n;
	__m128i *x128,*y128,*z128,dxsss,pxds,setnum,cum,pluscum;
	int32_t *result;
	*tot_metric = -99999999;


	time_diff = rdtsc_oai();
//	d05 = (int16_t*)malloc(sizeof(int16_t)*248*504);
	  ...
	  for (flip=0; flip<2; flip++) {	  //  d0/d5 flip in RX frame
		for (phase=0; phase<7; phase++) { // phase offset between PSS and SSS
		  for (Nid1 = 0 ; Nid1 <= 167; Nid1++) {  // 168 possible Nid1 values
			...
			if (flip==0) {
			  d05 = &dd05_sss[248*(Nid2 + (Nid1*3))]; //add d05

			} else {
			  d05 = &dd50_sss[248*(Nid2 + (Nid1*3))]; 
			...
			x128 = (__m128i*) sss05;
			y128 = (__m128i*) d05;
			z128 = (__m128i*) (int32_t*)&phase_ri;
				
			// This is the inner product using one particular value of each unknown parameter
			for (n=0; n<32; n++) {
		
			  dxsss = _mm_sign_epi16(x128[0], y128[0]);
			  pxds = _mm_madd_epi16(z128[phase], dxsss);
			  setnum = _mm_srai_epi32(pxds, 19);
			  cum = _mm_add_epi32(cum, setnum);
			...
			y128++;
			 ...
			 pluscum = _mm_hadd_epi32(cum, cum);
			 pluscum = _mm_hadd_epi32(pluscum, pluscum);


			result = (int32_t*)&pluscum[0];
			metric += (int16_t) result;
